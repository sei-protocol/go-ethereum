...
func NewEVM(
    blockCtx BlockContext, statedb StateDB, chainConfig *params.ChainConfig, config Config,
...
func (evm *EVM) Call(
    caller common.Address, addr common.Address, input []byte, gas uint64, value *uint256.Int,
    ...
    if isPrecompile {
        ret, gas, err = RunPrecompiledContract(
            p, evm, caller, caller, input, gas, value.ToBig(), evm.Config.Tracer, evm.interpreter.readOnly, false,
        )
    } else {
...
func (evm *EVM) CallCode(
    ...
    if p, isPrecompile := evm.precompile(addr); isPrecompile {
        ret, gas, err = RunPrecompiledContract(
            p, evm, caller, caller, input, gas, value.ToBig(), evm.Config.Tracer, evm.interpreter.readOnly, true,
        )
    } else {
        // Initialise a new contract and set the code that is to be used by the EVM.
        // The contract is a scoped environment for this execution context only.
...
func (evm *EVM) DelegateCall(
    originCaller common.Address, caller common.Address, addr common.Address, input []byte, gas uint64,
    value *uint256.Int,
) (ret []byte, leftOverGas uint64, err error) {
    // Invoke tracer hooks that signal entering/exiting a call frame
    if evm.Config.Tracer != nil {
    ...
    if p, isPrecompile := evm.precompile(addr); isPrecompile {
        ...
        ret, gas, err = RunPrecompiledContract(
            p, evm, originCaller, caller, input, gas, nil, evm.Config.Tracer, evm.interpreter.readOnly, true,
        )
    } else {
...
func (evm *EVM) StaticCall(caller common.Address, addr common.Address, input []byte, gas uint64) (ret []byte,
...
func (evm *EVM) create(
    caller common.Address, code []byte, gas uint64, value *uint256.Int, address common.Address, typ OpCode,
...
func (evm *EVM) initNewContract(contract *Contract, address common.Address) ([]byte, error) {
    ...
    if !evm.chainRules.IsEIP4762 {
        ...
        consumed, wanted := evm.AccessEvents.CodeChunksRangeGas(
            address, 0, uint64(len(ret)), uint64(len(ret)), true, contract.Gas,
        )
...
// Create creates a new contract using code as deployment code.
func (evm *EVM) Create(caller common.Address, code []byte, gas uint64, value *uint256.Int) (ret []byte,
...
func (evm *EVM) Create2(
    caller common.Address, code []byte, gas uint64, endowment *uint256.Int, salt *uint256.Int,
...
func (evm *EVM) CreateWithAddress(
    caller common.Address, code []byte, gas uint64, value *big.Int, address common.Address,
) (ret []byte, contractAddr common.Address, leftOverGas uint64, err error) {
    return evm.create(caller, code, gas, uint256.MustFromBig(value), address, CREATE)
}

func (evm *EVM) GetDeploymentCode(
    caller common.Address, code []byte, gas uint64, value *big.Int, address common.Address,
) ([]byte, uint64, error) {
...
func (evm *EVM) GetAccessEvents() *AccessEvents {
    return evm.TxContext.AccessEvents
}
